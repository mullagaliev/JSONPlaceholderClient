...
function usersReducer(state = defaultState, action) {
  ...
  switch (type) {
    case ACTION_TYPES.ALBUMS_BY_USER_ID_FETCH_REQUESTED:
      return StatusSwitcher(state,  REQUEST_STATUSES.REQUESTED);
    case ACTION_TYPES.ALBUMS_BY_USER_ID_FETCH_SUCCEEDED:
      const newState = Object.assign({}, state, {
        items: action.albums,
        total: action.albums.length,
      });
      return StatusSwitcher(newState, REQUEST_STATUSES.SUCCEEDED);
    case ACTION_TYPES.ALBUMS_BY_USER_ID_FETCH_FAILED:
